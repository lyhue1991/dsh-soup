...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 文件标签页视图（conversation.view 'dsh-tree-files'）：子 tab 条 + 预览区。
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css="@lyhue1991/dsh-tree/fileview.css"]') === null) {
      var fileviewTag = document.createElement('style')
      fileviewTag.dataset.plugin = '@lyhue1991/dsh-tree'
      fileviewTag.dataset.pluginCss = '@lyhue1991/dsh-tree/fileview.css'
    ...
    function rpc(action, args) {
      return fetch(new URL('/api/dsh-tree', hostBase()), {
        method: 'POST',
        // x-dsh-tree：宿主路由的强制头（防跨站简单请求伪造），
        // content-type 必须 application/json，两者缺一会被 403。
        headers: { 'content-type': 'application/json', 'x-dsh-tree': '1' },
    ...
    async function trackSession(sessionId, cwdHint) {
      ...
      setState({ cwd: cwd, selected: new Set(), renaming: null, newItem: null, menu: null, error: '', files: files })
    ...
    function updateUpload(name, loaded, total, idx, totalFiles) {
      setUploadEntry({ name: name, loaded: loaded, total: total, idx: idx, totalFiles: totalFiles, done: false })
    }

    function finishUploads(count, okCount, failedName) {
      setUploadEntry({
        name: '✓ 已上传 ' + (okCount === count ? count + ' 个文件' : okCount + '/' + count + ' 个文件' + (failedName ? '（' + failedName + ' 失败）' : '')),
        loaded: 0, total: 0, idx: 0, totalFiles: 0, done: true,
      })
    ...
    /** 下载文件：换一次性票据 URL → 浏览器原生流式另存为（任意大小）。 */
    async function downloadFile(path) {
      ...
      // 票据 URL 由宿主流式发送（Content-Disposition attachment），零前端内存放大
      var a = document.createElement('a')
      a.href = res.url
      a.download = res.name || baseName(path)
      document.body.appendChild(a)
      a.click()
      a.remove()
      setState({ notice: '已开始下载 ' + (res.name || baseName(path)), error: '' })
    }
    ...
    // ------------------------------------------------------------------
    // 文件视图（conversation.view 'dsh-tree-files'）：
        ...
        (s.uploads && s.uploads.length) ? create('div', { className: 'expl-uploads' },
          s.uploads.map(function (u, i) {
            ...
            return create('div', { key: i, className: 'expl-upload-row' + (u.done ? ' expl-upload-done' : '') },
              create('span', { className: 'expl-upload-glyph' }, u.done ? '✓' : '⬆'),
              create('span', { className: 'expl-upload-text' },
                u.done ? u.name
                  : '上传中 ' + (u.totalFiles > 1 ? '(' + u.idx + '/' + u.totalFiles + ') ' : '') + u.name,
              ),
    ...
    function apply(ctx) {
      ...
      slots.inject('conversation.view', function () {
        disposers.push(slots.register(
          { name: 'conversation.view', id: 'dsh-tree-files', order: 20, label: FILES_TAB_LABEL },
      ...
      slots.inject('conversation.input.dock', function () {
        disposers.push(slots.register(
          { name: 'conversation.input.dock', id: 'dsh-tree-hero-toggle', order: -10 },
      ...
      ctx.effect(
        ...
        'dsh-tree: explorer toggles + details panel + speed badge',
      )
    }
