...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function autoRefreshTick() {
      ...
      if (!state.open && state.files.list.length === 0) return
      // 用户正在操作（重命名/新建/右键菜单）时跳过本轮，避免打断输入
      if (state.renaming || state.newItem || state.menu) return
    ...
    function apply(ctx) {
      ...
      var disposers = []
      // 自动刷新心跳：每 3s 探一次 mtime；面板关且无预览时 tick 内直接返回（零请求）。
      // 首个 tick 只建基线，不会触发无谓重拉。
      if (timerRef && typeof timerRef.interval === 'function') {
        var stopAuto = timerRef.interval(function () { autoRefreshTick() }, AUTO_REFRESH_MS)
