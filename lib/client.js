...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ------------------------------------------------------------------
    // 速度徽标：输入框正上方显示「正在等待模型...」与彩色 t/s 吞吐徽标
    // 与宿主通信走同一 /api/dsh-tree 路由的 speed-status action。
    // ------------------------------------------------------------------
    ...
    function SpeedBadge(props) {
      ...
      var s = state[0]
      var setS = state[1]
      var dotsState = React.useState(1)
      var dots = dotsState[0]
      var setDots = dotsState[1]

      React.useEffect(function () {
        if (!timerRef) return
        var stop = timerRef.interval(function () { setDots(function (d) { return d >= 3 ? 1 : d + 1 }) }, 400)
      ...
      React.useEffect(function () {
        ...
        var poll = function () {
          rpc('speed-status', { sessionId: sessionId }).then(function (res) {
            if (!cancelled && res && res.ok) setS(res)
          }).catch(function () {})
        }
        poll()
      ...
      if (!s || s.phase === 'idle' || s.phase === 'done') return null

      // 悬浮徽标：容器 position:relative + height:0 不占流，内部徽标
      // position:absolute; bottom:100% 悬在输入区最顶端。脱离 flex 流，
      // 无论 goal/todo/queue 是否激活都与之互不重叠（不改 DSH 源码）。
      var containerStyle = {
        position: 'relative',
        height: 0,
        zIndex: 5,
      ...
      if (s.phase === 'waiting' || !s.tps) {
        return create('div', { style: containerStyle },
          create('div', { style: Object.assign({}, floatStyle, { fontSize: 13, color: 'var(--dsw-alias-label-caption)' }) },
            create('span', null, '正在等待模型' + new Array(dots + 1).join('.'))))
      }

      var tps = s.tps
      var bg = tps >= 50 ? '#53b3cb' : tps >= 30 ? '#9bc53d' : tps >= 15 ? '#f9c22e' : '#e01a4f'
      return create('div', { style: containerStyle },
        create('div', { style: Object.assign({}, floatStyle, { alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--dsw-alias-label-primary)' }) },
          create('span', { style: { display: 'flex', alignItems: 'center', gap: 4, color: 'var(--dsw-alias-label-caption)' } },
            create('svg', { width: 10, height: 10, viewBox: '0 0 10 10', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2, strokeLinecap: 'round', strokeLinejoin: 'round' },
              create('line', { x1: 5, y1: 1.5, x2: 5, y2: 8.5 }),
              create('polyline', { points: '2 6 5 8.5 8 6' }),
            ),
            String(Math.round(s.tokens)),
          ),
          create('span', { style: { marginLeft: 6, padding: '1px 6px', borderRadius: 4, background: bg, color: '#fff', fontSize: 11, fontWeight: 500 } }, tps.toFixed(1) + ' t/s'),
        ))
    }
