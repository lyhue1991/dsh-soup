...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function trackSession(sessionId, cwdHint) {
      ...
      // 会话切换：预览列表按新 cwd 裁剪，范围外的 tab 自动关闭
      var files = pruneFilesToScope(state.files, cwd)
      setState({ cwd: cwd, selected: new Set(), renaming: null, newItem: null, menu: null, files: files })
    ...
    /** 下载文件：base64 拉取 → Blob → 触发浏览器另存为。 */
    async function downloadFile(path) {
      setState({ menu: null })
      var res = await rpc('download', { path: path })
      if (!res || !res.ok) {
        setState({ error: (res && res.error) || '下载失败' })
        return
      }
      fetch('data:application/octet-stream;base64,' + res.data)
        ...
        .catch(function (e) {
          setState({ error: '下载失败: ' + String((e && e.message) || e) })
        })
    }
    ...
    async function openFileInTab(path) {
      ...
      activateFilesView()
      var res = await rpc('read', { path: path })
