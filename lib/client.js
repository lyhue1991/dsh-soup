...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var GOAL_CSS =
      ...
      '.dsh-goal-budgetrow{display:flex;align-items:center;gap:8px;margin-top:2px;}' +
      '.dsh-goal-budget{flex:1;min-width:0;height:30px;font-size:13px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);border-radius:6px;padding:0 8px;outline:none;font-family:var(--ds-font-family-code);box-sizing:border-box;}' +
      '.dsh-goal-budget:focus{border-color:var(--dsw-alias-state-business-primary);}' +
      '.dsh-goal-budget:disabled{opacity:.5;}' +
      '.dsh-goal-unlimited{display:inline-flex;align-items:center;gap:4px;font-size:12px;color:var(--dsw-alias-label-secondary);white-space:nowrap;cursor:pointer;flex:none;}'
    ...
    // （状态圆点 + 标签 + 用时/token 预算 meta + 按钮 + 完整多行 objective）。
    ...
    var GOAL_STATUS = {
      ...
      budget_limited: { color: '#e01a4f', label: '预算耗尽' },
    }
     ...
     * 数据走 dsh-tree 桥（goal-view 轮询）：显示 token 预算/用量而非轮数；
     * 动词走 goal-action（暂停/恢复/编辑含预算/清除）。
     */
    function GoalCard(props) {
      var sessionId = props.sessionId || (props.session && props.session.id)

      // viewState: null = 首帧加载中；{ goal: <object|null> } = 有/无目标
      var viewState = React.useState(null)
      var editing = React.useState(false)
      var draft = React.useState('')
      var budgetDraft = React.useState('')
      var unlimited = React.useState(false)
      var pending = React.useState(false)
      var error = React.useState(null)
      var pendingRef = React.useRef(false)

      var view = viewState[0]
      var goal = view === null ? undefined : view.goal
      var loaded = view !== null

      // 轮询 goal-view（每 1s 同时刷新用时），与速度徽标同款模式。
      React.useEffect(function () {
        if (!sessionId || !timerRef) return
        var cancelled = false
        var poll = function () {
          rpc('goal-view', { sessionId: sessionId }).then(function (res) {
            if (!cancelled) viewState[1](res && res.ok ? { goal: res.goal || null } : { goal: null })
          }).catch(function () {})
        }
        poll()
        var stop = timerRef.interval(poll, 1000)
        return function () { cancelled = true; stop() }
      }, [sessionId])
      ...
      // 首帧加载中、无 goal、已完成：都不渲染（与默认 GoalBar 行为一致）。
      if (!loaded || goal === null || goal.phase === 'complete') return null

      var status = GOAL_STATUS[goal.phase] || { color: 'var(--dsw-alias-label-caption)', label: goal.phase }
      var elapsed = formatElapsed((goal.timeUsedSeconds || 0) * 1000)
      var tokensMeta = formatTokens(goal.tokensUsed || 0)
        + (goal.tokenBudget === null || goal.tokenBudget === undefined
          ? ' t' : ' / ' + formatTokens(goal.tokenBudget) + ' t')
      var meta = elapsed + ' · ' + tokensMeta
      ...
      function runAction(payload) {
        ...
        pending[1](true)
        error[1](null)
        return rpc('goal-action', Object.assign({ sessionId: sessionId }, payload)).then(function (result) {
          pendingRef.current = false
          pending[1](false)
          if (result && result.ok) {
            viewState[1]({ goal: result.goal || null })
          } else {
            error[1]((result && result.error) || '操作失败')
          }
          return result
        }).catch(function (e) {
          pendingRef.current = false
      ...
      function handleSave() {
        ...
        if (trimmed === '') return
        var payload = { action: 'edit', objective: trimmed }
        if (unlimited[0]) {
          payload.token_budget = null
        } else if (budgetDraft[0] !== '') {
          var n = Number(budgetDraft[0])
          if (!Number.isFinite(n) || n <= 0 || !Number.isSafeInteger(n)) {
            error[1]('token 预算必须是正整数；留空保持不变，勾选无上限可移除上限')
            return
          }
          payload.token_budget = n
        }
        runAction(payload).then(function (result) {
          if (result && result.ok) {
            editing[1](false)
            budgetDraft[1]('')
            unlimited[1](false)
      ...
      function openEdit() {
        ...
        budgetDraft[1](goal.tokenBudget === null || goal.tokenBudget === undefined ? '' : String(goal.tokenBudget))
        unlimited[1](goal.tokenBudget === null || goal.tokenBudget === undefined)
        editing[1](true)
      }
      ...
      function textBtn(label, onClick, primary, disabled) {
        return create('button', {
          key: label, type: 'button', disabled: disabled === true,
      ...
      if (editing[0]) {
        return create('div', { className: 'dsh-goal-card', 'data-dsh-goal-card': '' },
          ...
          create('div', { className: 'dsh-goal-budgetrow' },
            create('input', {
              className: 'dsh-goal-budget',
              type: 'number', min: 1, step: 1,
              placeholder: 'token 预算（留空保持不变）',
              'aria-label': 'token 预算',
              value: budgetDraft[0],
              disabled: unlimited[0],
              onChange: function (e) { budgetDraft[1](e.target.value) },
            }),
            create('label', { className: 'dsh-goal-unlimited' },
              create('input', {
                type: 'checkbox',
                checked: unlimited[0],
                onChange: function (e) { unlimited[1](e.target.checked) },
              }),
              ' 无上限',
            ),
          ),
          ...
          create('div', { className: 'dsh-goal-editbtns' },
            ...
            textBtn('保存', handleSave, true, pending[0] || draft[0].trim() === ''),
          ),
      ...
      return create('div', { className: 'dsh-goal-card', 'data-dsh-goal-card': '' },
        create('div', { className: 'dsh-goal-head' },
          ...
          create('div', { className: 'dsh-goal-actions' },
            goal.phase === 'active' && iconBtn('暂停目标', function () { runAction({ action: 'pause' }) }, false, ICON_PAUSE),
            goal.phase === 'paused' && iconBtn('恢复目标', function () { runAction({ action: 'resume' }) }, true, ICON_PLAY),
            iconBtn('编辑目标', openEdit, false, ICON_EDIT),
            iconBtn('清除目标', function () { runAction({ action: 'clear' }) }, false, ICON_TRASH),
          ),
        ),
    ...
    function apply(ctx) {
      ...
      slots.inject('conversation.input.dock', function () {
        ...
        // 数据/动词全走 dsh-tree 桥（goal-view 轮询 / goal-action）。
        disposers.push(slots.register(
