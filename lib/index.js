...
export function apply(ctx) {
  ...
  const GOAL_BLOCK_AFTER_ROUNDS = 3
  ...
  const goalStorePath = joinPath(root, '.dsh-tree-goals.json')
  const goalStoreReady = (async () => {
    try {
      const raw = await readFile(goalStorePath, 'utf8')
      const saved = JSON.parse(raw)
      if (saved && typeof saved === 'object' && !Array.isArray(saved)) {
        for (const [id, value] of Object.entries(saved)) {
          if (typeof id === 'string' && value && typeof value === 'object') {
            const budget = value.tokenBudget === null ? null : Number(value.tokenBudget)
            ...
            if ((budget === null || (Number.isSafeInteger(budget) && budget > 0))
              ...
              && ['active', 'paused', 'blocked', 'complete', 'budget_limited'].includes(status)) {
              goalState.set(id, { tokenBudget: budget, tokensUsed, status, createdAt })
            }
          }
        }
      }
    } catch (err) {
      if (err && err.code !== 'ENOENT') console.error('[dsh-tree] goal 状态读取失败:', (err && err.message) || err)
    }
  })()

  async function persistGoalState() {
    const data = Object.fromEntries(goalState)
    await mkdir(root, { recursive: true })
    await writeFile(goalStorePath, JSON.stringify(data, null, 2) + '\n', 'utf8')
  }
  ...
  function resolveAgent(exec) {
    ...
    if (ctx.agents.get(agent.id) !== agent || agent.status !== 'running'
      ...
      throw toolError('goal 工具需要精确的存活调用 agent 在活动 driver 内', 'GOAL_TOOL_DRIVER_REQUIRED')
    }
    return agent
  }

  function openTurnEvents(agent) {
    const events = (agent.session && agent.session.events) || []
    for (let i = events.length - 1; i >= 0; i--) {
      const event = events[i]
      if (!event) continue
      if (event.type === 'turn/end') return null
      if (event.type === 'turn/start') return events.slice(i + 1)
    }
    return null
  }
  ...
  function hasDirectHuman(agent) {
    ...
    const events = openTurnEvents(agent)
  ...
  function hasGoalRound(agent, goal) {
    const events = openTurnEvents(agent)
    if (!events) return false
    return events.some(event => event.type === 'user/message'
      && event.data && event.data.source && event.data.source.kind === 'goal'
  ...
  function goalView(agent, sid) {
    ...
    return {
      ...
      timeUsedSeconds: Math.max(0, Math.round((Date.now() - native.createdAt) / 1000)),
  ...
  function accumulateGoalUsage(sid, delta) {
    ...
    persistGoalState().catch(err => {
      console.error('[dsh-tree] goal 状态写入失败:', (err && err.message) || err)
    })
  }
  ...
  // 注意：output.schema 需为 valueSchemaSpecToJsonSchema 的**编译后**格式
  // （每层顶层 required 数组、属性无内联 required），才能通过 register 的
  // assertSupportedJsonSchema 校验（原生 defineTool 会先编译 DSL，这里手工编译）。
  const GOAL_OUTPUT_SCHEMA = {
    oneOf: [
      { type: 'object', additionalProperties: false, properties: { goal: { type: 'null' } }, required: ['goal'] },
      {
        ...
        properties: {
          goal: {
            ...
            properties: {
              ...
              phase: { type: 'string', enum: ['active', 'paused', 'blocked', 'complete', 'budget_limited'] },
              tokenBudget: { oneOf: [{ type: 'integer' }, { type: 'null' }] },
            ...
            required: ['id', 'revision', 'objective', 'phase', 'tokenBudget', 'tokensUsed', 'timeUsedSeconds'],
          },
  ...
  function updateGoalTool() {
    return {
      ...
      async execute(args, exec) {
        ...
        switch (action) {
          case 'edit': {
            if (!hasDirectHuman(agent)) throw toolError('edit 需要顶层 agent 的直接人类轮', 'GOAL_TOOL_AUTHORITY_REQUIRED')
            if (args.objective === undefined && args.token_budget === undefined) {
              throw toolError('edit 需要 objective 或 token_budget', 'GOAL_INVALID_EDIT')
            }
            const replacementBudget = args.token_budget === undefined
              ? undefined
              : normalizeBudget(args.token_budget)
            const objective = args.objective === undefined
              ? undefined
              : String(args.objective).trim()
            if (objective !== undefined && !objective) throw toolError('objective 不能为空', 'GOAL_INVALID_OBJECTIVE')
            if (objective !== undefined) ctx.goals.edit(agent, ref, { objective })
            const gs = goalState.get(sid) || { tokenBudget: null, tokensUsed: 0, status: 'active', createdAt: Date.now() }
            if (replacementBudget !== undefined) {
              gs.tokenBudget = replacementBudget
              if (gs.status === 'budget_limited' && (replacementBudget === null || gs.tokensUsed < replacementBudget)) gs.status = 'active'
            }
          ...
          case 'pause': {
            if (!hasDirectHuman(agent)) throw toolError('pause 需要顶层 agent 的直接人类轮', 'GOAL_TOOL_AUTHORITY_REQUIRED')
          ...
          case 'resume': {
            if (!hasDirectHuman(agent)) throw toolError('resume 需要顶层 agent 的直接人类轮', 'GOAL_TOOL_AUTHORITY_REQUIRED')
            const gs = goalState.get(sid) || { tokenBudget: null, tokensUsed: 0, status: 'active', createdAt: Date.now() }
          ...
          case 'complete': {
            if (!hasDirectHuman(agent) && !hasGoalRound(agent, current)) throw toolError('complete 需要直接人类轮或当前目标轮', 'GOAL_TOOL_AUTHORITY_REQUIRED')
          ...
          case 'blocked': {
            if (!hasDirectHuman(agent) && !hasGoalRound(agent, current)) throw toolError('blocked 需要直接人类轮或当前目标轮', 'GOAL_TOOL_AUTHORITY_REQUIRED')
            if (!hasDirectHuman(agent) && current.roundsStarted < GOAL_BLOCK_AFTER_ROUNDS) {
              throw toolError(`blocked 需要至少 ${GOAL_BLOCK_AFTER_ROUNDS} 个连续 goal 轮`, 'GOAL_TOOL_BLOCK_THRESHOLD')
            }
  ...
  function installGoalTools(agent) {
    ...
    const cleanup = agent.ctx.effect(() => {
      ...
      return () => {
        for (const d of disposes) { try { d() } catch (err) { /* ignore */ } }
        if (installedCleanups.get(agent) === cleanup) installedCleanups.delete(agent)
      }
    }, 'dsh-tree: goal tools')
    installedCleanups.set(agent, cleanup)
  }

  const installedAgents = new WeakSet()
  const installedCleanups = new WeakMap()
  ctx.effect(() => {
      ...
      case 'goal-action': {
        ...
        try {
          switch (action) {
            case 'pause': {
              ctx.goals.pause(agent, ref)
              const pausedState = goalState.get(sid)
            ...
            case 'complete': {
              ctx.goals.complete(agent, ref)
              const completeState = goalState.get(sid)
              if (completeState) completeState.status = 'complete'
              break
            }
            case 'blocked': {
              ctx.goals.block(agent, ref, { code: 'user', message: '用户标记为受阻' })
              const blockedState = goalState.get(sid)
              if (blockedState) blockedState.status = 'blocked'
              break
            }
            case 'edit': {
              if (args.objective === undefined && args.token_budget === undefined) {
                return { ok: false, error: 'edit 需要 objective 或 token_budget' }
              }
              if (args.objective !== undefined && !String(args.objective).trim()) {
                return { ok: false, error: 'objective 不能为空' }
              }
              const replacementBudget = args.token_budget === undefined
                ? undefined
                : normalizeBudget(args.token_budget)
