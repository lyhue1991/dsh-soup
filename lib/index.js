...
import { rename, mkdir, writeFile, appendFile, open as openFile, readFile, rm, stat, realpath } from 'node:fs/promises'
...
export function apply(ctx) {
  ...
  // ------------------------------------------------------------------
  // 文件预览/编辑：read（文本/图片/二进制三态）+ write（mtime/size 防覆盖保存）
  // ------------------------------------------------------------------
  ...
  /** HTML 预览的单个同目录静态资源读取上限（与官方预览一致）。 */
  const HTML_ASSET_MAX_BYTES = 4 * 1024 * 1024
  ...
  /**
   * 保存预览编辑：mtime/size 防覆盖 + 同目录临时文件原子替换。
   * fsync 无法通过 node:fs/promises 便携获得；rename 前先 close 已落盘，
   * 已满足 UI 编辑场景的崩溃一致性下限，并保留写失败时原文件不变。
   */
  async function writeFileForEditor(pathArg, contentArg, expectedMtime, expectedSize, sessionIdArg) {
    const rp = await confine(pathArg, '保存路径', sessionIdArg)
    const content = typeof contentArg === 'string' ? contentArg : ''
    const bytes = Buffer.from(content, 'utf8')
    if (bytes.length > TEXT_MAX_BYTES) {
      return { ok: false, error: `内容超过 ${TEXT_MAX_BYTES} 字节，禁止保存` }
    }
    let before
    try {
      before = await stat(rp)
    } catch (err) {
      if (err && err.code === 'ENOENT') return { ok: false, conflict: true, error: '文件已被删除，保存已取消' }
      return { ok: false, error: `无法读取原文件: ${String((err && err.message) || err)}` }
    }
    if (before.isDirectory()) return { ok: false, error: '目标是目录，不支持保存' }
    if (expectedMtime !== undefined && (Number(expectedMtime) !== before.mtimeMs
      || Number(expectedSize) !== before.size)) {
      return {
        ok: false,
        conflict: true,
        error: '文件已被其他程序修改，请重新加载后再保存',
        mtime: before.mtimeMs,
        size: before.size,
      }
    }
    const tempPath = `${rp}.dsh-soup-${process.pid}-${Date.now()}-${randomBytes(4).toString('hex')}.tmp`
    try {
      await writeFile(tempPath, bytes)
      await rename(tempPath, rp)
      const after = await stat(rp)
  ...
  /**
   * 读取文件供 tab 展示：图片返回 base64，文本返回内容与版本（可截断），
  ...
  async function handleAction(body) {
    ...
    switch (action) {
      ...
      case 'read-related': {
        // HTML 预览专用：只可读取 rootPath 所在目录内的相对资源。iframe 从不
        // 拿到这个能力；客户端仅用它打包 css / 传统 js 到 Blob 文档中。
        const rootPath = await confine(args.path, 'HTML 文件路径', sessionIdArg)
        ...
        const rp = await confine(candidate, 'HTML 依赖路径', sessionIdArg)
        if (!within(base, rp)) return { ok: false, error: 'HTML 依赖超出文档目录' }
        let st
        try { st = await stat(rp) } catch (err) {
          return { ok: false, error: '无法读取 HTML 依赖: ' + String((err && err.message) || err) }
        }
        if (st.isDirectory()) return { ok: false, error: 'HTML 依赖是目录' }
        if (st.size > HTML_ASSET_MAX_BYTES) return { ok: false, error: 'HTML 依赖超过 ' + HTML_ASSET_MAX_BYTES + ' 字节' }
        try {
          return { ok: true, data: (await readFile(rp)).toString('base64'), size: st.size }
        } catch (err) {
          return { ok: false, error: '无法读取 HTML 依赖: ' + String((err && err.message) || err) }
        }
      }
