...
import { rename, mkdir, writeFile, open as openFile, stat, realpath } from 'node:fs/promises'
...
export function apply(ctx) {
   ...
   */
  async function confine(rawPath, label) {
    const name = label || '路径'
    ...
    for (const base of await allowedBases()) {
  ...
  async function handleAction(body) {
    ...
    switch (action) {
      ...
      case 'list': {
        const rp = await confine(String(args.path || '') || root, '目录')
        let target
        let entries
        try {
      ...
      case 'move': {
        ...
        const fromRp = await confine(from, '源路径')
        const toRp = await confine(to, '目标路径')
      ...
      case 'create': {
        ...
        const dirRp = await confine(dir, '目录')
        // 二次围栏：name 若与既有符号链接同名，lenient realpath 会解析出
        // 链接真实指向，越界即拒绝。
        const target = await confine(joinPath(dirRp, newName), '创建路径')
        try {
          if (isDir) {
      ...
      case 'upload': {
        ...
        const dirRp = await confine(dir, '目录')
        const target = await confine(joinPath(dirRp, fileName), '上传路径')
        try {
      ...
      case 'read': {
        const rp = await confine(args.path, '文件路径')
        return readFileForView(rp)
      }
      case 'download': {
        const rp = await confine(args.path, '下载路径')
        let dst
        try {
          dst = await stat(rp)
        } catch (err) {
        ...
        if (dst.size > DOWNLOAD_MAX_BYTES) {
          return { ok: false, error: `文件超过下载上限（${Math.round(DOWNLOAD_MAX_BYTES / 1048576)}MB）` }
        }
        try {
          const fh = await openFile(rp, 'r')
          try {
            const buf = Buffer.alloc(dst.size)
            await fh.read(buf, 0, dst.size, 0)
            return { ok: true, name: path.basename(rp), data: buf.toString('base64'), size: dst.size }
          } finally {
            await fh.close()
          }
        } catch (err) {
   ...
   * - 必须携带 x-dsh-tree: 1 且 content-type 为 application/json：
  ...
  function gate(req, res) {
    ...
    if (headers['x-dsh-tree'] !== '1' || !contentType.startsWith('application/json')) {
