...
export function apply(ctx) {
  ...
  ctx.effect(
    ...
    'dsh-tree: download ticket sweep',
  )
  ...
  ctx.effect(
    () => ctx.webServer.register({ kind: 'exact', path: '/api/dsh-tree/dl', handler: dlHandler }),
    'dsh-tree: streaming download route',
  )
  ...
  async function handleAction(body) {
    ...
    switch (action) {
      ...
      case 'download': {
        ...
        // 真正的字节由 /api/dsh-tree/dl 流式发送——任意大小文件零内存放大。
        ...
        return { ok: true, name: path.basename(rp), size: dst.size, url: `/api/dsh-tree/dl?t=${token}` }
      }
      case 'speed-status': {
