 ...
 * 宽度对齐宿主 details 合同：默认 300px、可拖 300–520px（直接改写 shell 网格
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 速度徽标（区域三 · 🧂 盐）实现在 client/speed-badge.js：注册为
    // conversation.session.header.actions 槽位组件（与官方 Agent 预设标签
    // 同槽，排其右侧）；timerRef 为共享模块级计时引用（apply 里赋值），
    // 经 getter 注入。
    ...
    function apply(ctx) {
      ...
      slots.inject('conversation.input.dock', function () {
        ...
        // 与官方 jobs 插件同款两段式注册：inject 把条目绑定到会话作用域
        // （缺这层时条目不进入渲染树——这正是徽标此前不可见的根因）。
        slots.inject('conversation.session.header.actions', function () {
          disposers.push(slots.register(
            { name: 'conversation.session.header.actions', id: 'dsh-speed-badge', order: 10, locale: NS, label: function () { return T('speed.label') } },
