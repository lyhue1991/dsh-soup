...
export function createPreviewRenderers(ctx) {
  ...
  // ------------------------------------------------------------------
  // Markdown 相对图片 → 官方 /api/file 文件路由词汇表（pathImages）。
  // DSH 的 MarkdownText 渲染器只放行 http/https/mailto URL 与
  // pathImages/fileImages 词汇表改写结果：桌面端 WebView 的页面基址是
  // 自定义协议 dsh-app://app，把 src 改写成自家 img 端点会被
  // sanitizeUrl 直接丢弃（alt 文本兜底）；浏览器打开时才是 http。
  // 因此改为向 MarkdownText 提供 pathImages 词汇表：相对路径以预览
  // 文件所在目录为基解析成绝对路径，经官方 api/file 路由（浏览器为
  // http 同源、桌面为 dsh-app://app/api/file，均在官方白名单内，
  // 鉴权 cookie 由协议转发器注入）返回给 <img> 加载。
  // ------------------------------------------------------------------
  ...
  /** pathImages.resolve：本地路径 → 官方 api/file URL；非本地路径返回 undefined。 */
  function localPathImageUrl(baseDir, value) {
    var raw = String(value || '')
    // 与 DSH localPathMediaUrl 同规：剥离 ?# 后缀，URL 转义解码一次。
    var bare = raw.split(/[?#]/)[0]
    var decoded
    try { decoded = decodeURIComponent(bare) } catch (e) { return undefined }
    if (!decoded || /[\u0000-\u001f\u007f]/.test(decoded)) return undefined
    // 协议 URL（http/https/data 等）与 UNC 路径不属于本地路径词汇表，
    // 留给官方 imageSource 处理；Windows 盘符路径按官方例外放行。
    if (/^[a-z][a-z\d+.-]*:/i.test(decoded) && !/^[a-z]:[\\/]/i.test(decoded)) return undefined
    if (/^[/\\]{2}/.test(decoded)) return undefined
    var abs = decoded.charAt(0) === '/' || /^[a-z]:[\\/]/i.test(decoded)
      ? decoded
      : posixResolve(baseDir || '/', decoded)
    try {
      return new URL('api/file?path=' + encodeURIComponent(abs), document.baseURI).href
  ...
  function MarkdownPreview(props) {
    ...
    if (MarkdownText) {
      ...
      // 相对图片经 pathImages 词汇表交给官方 api/file 路由（见 localPathImageUrl 注释）。
      var pathImages = props.baseDir
        ? { resolve: function (value) { return localPathImageUrl(props.baseDir, value) } }
        : undefined
      var md = text
      ...
      function pushNormal() {
        ...
        if (value.trim() !== '') chunks.push(create(MarkdownRenderBoundary, { key: 'md-' + chunks.length, text: value }, create(MarkdownText, { text: value, pathImages: pathImages })))
        normal = []
      }
      function pushTable(headerLine) {
        ...
        var renderCell = function (cell, key) {
          return create(MarkdownRenderBoundary, { key: key, text: cell }, create(MarkdownText, { text: cell, pathImages: pathImages }))
        }
        var head = create('tr', null, headers.map(function (cell, hi) {
  ...
  return {
    ...
    localPathImageUrl: localPathImageUrl,
