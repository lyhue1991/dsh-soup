...
一个可安装的 DSH（DeepSeek Harness）插件：在会话右侧以**并列网格列**的形式提供项目文件树，与左侧工作区同款配色、同宽（默认 280px，可拖拽 264–420px）。
...
cd dsh-tree
# 纯 JS 产物，无需构建：
#   lib/index.js  —— 宿主半区（webServer 路由 /api/dsh-tree）
...
- **动作**：`root` / `sessionCwd` / `list` / `open`（`/usr/bin/open`）/ `trash`（Finder osascript）/ `move`（`/bin/mv`）/ `create`（`mkdir -p` / `touch`）/ `upload`（`base64 -D`）。
