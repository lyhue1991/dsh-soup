...
if (!clientSource.includes(".expl-btn{width:26px;height:26px;")) throw new Error('explorer header buttons must be 26px icon buttons')
if (!clientSource.includes('.dfv-btn{flex:none;display:inline-flex')) throw new Error('preview reload button must be flex icon+label')
...
if (!clientSource.includes('[data-sidebar-right-expand],[data-sidebar-right-expand-placeholder]{display:none!important;}')) throw new Error('official rightbar expand control must be hidden')
if (!clientSource.includes('data-dsh-soup-preview="1"')) throw new Error('reading-mode css marker missing')
if (!clientSource.includes(".dfv-header{position:sticky;top:0;")) throw new Error('preview tabs/path header must stay sticky')
...
if (!clientSource.includes("'explorer.overwriteConfirm': '文件“{name}”已存在，是否覆盖？'")) throw new Error('overwrite confirmation must have Chinese translation')
if (!clientSource.includes("'explorer.overwriteConfirm': 'The file \"{name}\" already exists. Overwrite it?'")) throw new Error('overwrite confirmation must have English translation')
...
if (!clientSource.includes("'explorer.overwrite': '覆盖'")) throw new Error('overwrite button must have Chinese translation')
if (!clientSource.includes("'explorer.overwrite': 'Overwrite'")) throw new Error('overwrite button must have English translation')
